...
2. 提出 57 维时序特征工程方法（融合统计特征与时序动力学特征），在单全局 XGBoost 架构下达到 Recall=86.05%（th=0.075）、F1=0.838、AUC=0.988、PR-AUC=0.893（随机基线 0.0465 的 19.2 倍）；与个性化联邦 Transformer 基线（Recall=73.56%、F1=0.578）相比，在阈值无关指标（PR-AUC=0.893 vs 随机基线 19.2 倍）与同阈值特征消融（th=0.5 下 Recall 58.14%→65.12%，PR-AUC 0.862→0.893）上更具优势，证明特征工程可替代复杂个性化架构解决跨站点泛化问题；
...
**个性化联邦学习**：KC 等人（Nature Communications, 2026）针对跨站点泛化问题提出了基于 Hyper-Network 的个性化联邦学习方法，在深圳充电站数据集上达到 Recall=73.56%（Acc=92.43%、F1=0.578）。本文在相同数据集上验证了特征工程路线的有效性——无需联邦架构，通过 57 维时序特征与阈值校准，即可在 Recall=86.05% 的工作点（th=0.075）与 AUC=0.988 上达到可比乃至更优的性能。
...
5. **基线对比的工作点差异**：本文与 KC 等人的对比存在决策阈值差异（本文 th=0.075 vs 原论文未披露阈值），且窗口级 Transformer 复现（3 特征、15 epochs）超参搜索不及原论文充分，可能低估其性能。为缓解该问题，本文同时报告默认阈值（th=0.5）、阈值无关指标（AUC、PR-AUC）与同阈值特征消融（th=0.5 下 37→57 维 Recall 58.14%→65.12%、PR-AUC 0.862→0.893）作为公平对比依据；未来将按原论文完整设置复现个性化联邦框架以做严格对比。
...
2. 通过 57 维时序特征工程（统计特征 + 时序动力学特征），在单全局 XGBoost 架构下达到 Recall=86.05%（th=0.075）、F1=0.838、AUC=0.988、PR-AUC=0.893，在阈值无关指标（AUC、PR-AUC）与同阈值特征消融（th=0.5 下 37→57 维 Recall 58.14%→65.12%、PR-AUC 0.862→0.893）上优于个性化联邦基线（Nature Communications, Recall=73.56%、F1=0.578），证明特征工程可替代复杂个性化架构解决跨站点泛化问题；
