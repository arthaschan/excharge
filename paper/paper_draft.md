...
电动汽车充电基础设施的快速扩张对运营监控与故障诊断提出了严峻挑战。深度学习方法在充电站异常检测中取得了先进性能，但现有工作普遍缺乏对**跨站点泛化能力**与**监管合规要求**的系统性回应。本文基于深圳 30 座充电站真实运营数据（155.6 万采样点，2020—2023），提出一个面向充电站网络的可解释异常检测框架，主要包含三个模块：（1）融合时序动力学的**多尺度特征工程**（57 维特征）；（2）基于阈值校准的 **XGBoost 异常分类器**；（3）**TreeSHAP 事后归因**模块，将每次预测分解至具体传感器特征。在新站点（owner 7-8）跨域测试中，本文方法达到 Recall=86.05%、Precision=81.62%、F1=0.838、AUC=0.988，在无需任何个性化联邦学习的前提下，**全面超越了个性化联邦 Transformer 基线**（Nature Communications, Recall=73.56%）。SHAP 归因揭示了物理可解释的"故障指纹"：异常充电表现为大电流高功率段突然中断（末端 SOC 78.7% vs 正常 94.3%，末端功率 42.1 kW vs 15.3 kW），且枪温并非故障的主要驱动因素。本文进一步提出面向欧盟《人工智能法案》（EU AI Act）高风险系统的合规框架，包括逐条义务映射、审计日志模板与模型卡片（Model Card），为关键基础设施 AI 系统的透明性与可追责性提供了完整的技术落地方案。
...
2. 提出 57 维时序特征工程方法（融合统计特征与时序动力学特征），在单全局 XGBoost 架构下达到 Recall=86.05%、F1=0.838，**超越个性化联邦 Transformer 基线（+12.5pp）**，证明特征工程可替代复杂个性化架构解决跨站点泛化问题；
...
**深度学习方法**：MTAD-GAT [Zhao et al., ICDM 2020] 使用图注意力网络联合建模时序与特征依赖；GDN [Deng & Hooi, AAAI 2021] 通过学习传感器关系图实现异常检测；AnomalyBERT [ICLR 2023 Workshop] 将 Transformer 架构引入时序异常检测。然而，这些方法均在新站点泛化上表现脆弱（见本文 §4.3 实验）。

**个性化联邦学习**：KC 等人（Nature Communications, 2026）针对跨站点泛化问题提出了基于 Hyper-Network 的个性化联邦学习方法，在深圳充电站数据集上达到 Recall=73.56%。本文在相同数据集上验证了特征工程路线的有效性——无需联邦架构，通过 57 维时序特征即可将 Recall 提升至 86.05%。
...
本文使用 KC 等人（Nature Communications, 2026）发布的深圳充电站公开数据集，来源为深圳autosun科技有限公司与香港理工大学，数据托管于 Mendeley 与 Zenodo（DOI: 10.5281/zenodo.17423221）。
...
- 训练集：owner 1-4，12,484 条有效充电序列（≥30 个采样点），802 故障
...
**表 1：跨新站点测试集（owner 7-8）性能对比**

| 方法 | Recall | Precision | F1 | AUC | 训练时间 |
|------|--------|-----------|-----|-----|---------|
| RandomForest | 4.65% | 75.00% | 0.088 | 0.959 | 1.1s |
| MLP | 38.76% | 67.57% | 0.493 | 0.955 | 1.3s |
| **XGBoost（v1 基准，37 特征）** | 58.14% | 89.29% | 0.704 | 0.989 | 0.6s |
| **XGBoost（v1+v2，57 特征）** | **86.05%** | **81.62%** | **0.838** | **0.988** | 0.7s |
| Transformer（窗口级）[KC et al., 2026] | 2.79% | 12.50% | 0.046 | 0.561 | 355s |
| 个性化联邦 Hyper-Network [KC et al., 2026] | 73.56% | — | 0.578 | — | — |

*Transformer 窗口级为本文在相同数据集上复现结果（30 epochs），个性化联邦数据取自原论文。*
...
本文最重要的发现是：**跨站点泛化可以通过特征工程而非模型架构创新来解决**。KC 等人（Nature Communications, 2026）将新站点性能衰退归因于站点间数据分布差异，并提出个性化联邦作为解决方案；本文则在相同的跨站点设定下，通过 57 维时序特征（特别是时序动力学特征 t2_max_jump、a_first_last_ratio 等）使单全局 XGBoost 的 Recall 达到 86.05%，超过个性化联邦 12.5pp。
...
2. 通过 57 维时序特征工程（统计特征 + 时序动力学特征），在单全局 XGBoost 架构下达到 Recall=86.05%、F1=0.838，**超越个性化联邦基线（Nature Communications, Recall=73.56%）**，证明特征工程可替代复杂个性化架构解决跨站点泛化问题；
...
[3] KC K, et al. Personalized federated learning with Hyper-Network for battery fault detection in EV charging. Nature Communications, 2026. DOI: 10.1038/s41467-025-67703-7.
...
[16] Zhou Z, et al. 1D Grad-CAM for Time Series Classification: Applications in EV Charging Anomaly Detection. 2024.
