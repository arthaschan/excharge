#!/usr/bin/env python3
"""Bi-LSTM + 注意力池化（全量数据，MPS 加速）—— 干净主模型。
vs 上一版(last+max pool)：改用可学习注意力池化，让模型自动关注关键时间步。
全量训练数据(13,505 序列, 642 故障)，不降采样。
"""
...
print('\n=== Bi-LSTM+Attn (full data, MPS) on Test (owner 7-8), th=0.5 ===', flush=True)
...
json.dump(res, open(f'{OUT}/routeC_bilstm_attn_results.json', 'w'), indent=2)
np.save(f'{OUT}/routeC_bilstm_attn_prob.npy', te_prob)
np.save(f'{OUT}/routeC_bilstm_attn_pred.npy', te_pred)
torch.save(best_state, f'{OUT}/routeC_bilstm_attn_model.pt')
